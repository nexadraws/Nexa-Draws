...
$('#adminContent').innerHTML=`<p class="eyebrow">NEXA DRAW ADMIN</p><h2>Competition Manager</h2><form id="competitionForm"><input type="hidden" name="existingId" value="${edit?.id||''}"><label class="field">Title<input name="title" required value="${edit?.title||''}"></label><label class="field">Price<input name="price" type="number" step="0.01" required value="${edit?.price||''}"></label><label class="field">Maximum entries<input name="max" type="number" required value="${edit?.max||''}"></label><label class="field">Entries sold<input name="sold" type="number" value="${edit?.sold||0}"></label><label class="field">Closing date<input name="closes" type="datetime-local" required value="${edit?.closes||''}"></label><label class="field">Image path / URL<input name="image" value="${edit?.image||''}"></label><label class="field">Upload image<input name="image_file" type="file" accept="image/jpeg,image/png,image/webp"></label><label class="field">Description<textarea name="description">${edit?.description||''}</textarea></label><label class="field">Skill Question<textarea name="skill_question">${edit?.skill_question||''}</textarea></label><label class="field">Option A<input name="skill_option_a" value="${edit?.skill_option_a||''}"></label><label class="field">Option B<input name="skill_option_b" value="${edit?.skill_option_b||''}"></label><label class="field">Option C<input name="skill_option_c" value="${edit?.skill_option_c||''}"></label><button class="btn primary full" type="submit">Save Competition</button></form>`;
